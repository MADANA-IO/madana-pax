...
function bundle() {
  ...
  const BUNDLE_DIRECTORY = './build/bundle/';
  const FILE_PREFIX = 'flattened_';

  readdirp({root: './contracts/', fileFilter: ['!Migrations.sol', '!.DS_Store']}, (
    // do something with each file
    (file) => {
      console.log(file.fullPath);
      bundleContract(file.fullPath).then(
        (data) => {
          writeBundledContract(file.name, data);
        }
      );
    }
  ), (error, files) => {
  ...
  async function bundleContract(path) {
    const result = await flatten([path]);
    return result;
  }

  function writeBundledContract(fileName, data) {
    fs.writeFile(BUNDLE_DIRECTORY + FILE_PREFIX + fileName, data, function (err) {
      if (err) {
